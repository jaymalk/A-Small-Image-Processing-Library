...
#ifndef CONVOLUTION_OB_H
#define CONVOLUTION_OB_H
    ...
    double * convolutionByMultiplication(vector<vector<float>> kernel, vector<vector<float>> matrix, bool convolution, bool padding=false) {
            ...
            int row = (matrix.size()-kernel.size()+1)*(matrix.size()-kernel.size()+1);
            int col = (kernel1D.size());

            double *A = convertToArray(matrix);
            ...
            call_open_blas(T, row, B, 1, col, C);
            free(T);
            free(A);
            free(B);
            return C;
